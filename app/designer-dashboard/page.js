...
export default function DesignerDashboardPage() {
  ...
  const serviceOptions = useMemo(() => {
    const uniqueServices = [
      ...new Set(designerCases.map((item) => item.service_type).filter(Boolean))
    ]
  ...
  const filteredCases = useMemo(() => {
    return designerCases.filter((item) => {
      const patientName =
  ...
  }, [designerCases, search, statusFilter, serviceFilter])

  const activeCasesCount = designerCases.filter(
    (item) => item.status !== "Delivered" && item.status !== "Cancelled"
  ).length

  const inDesignCount = designerCases.filter(
    (item) => item.status === "In Design"
  ).length

  const pendingInfoCount = designerCases.filter(
    (item) => item.status === "Pending Info"
  ).length

  const manufacturingCount = designerCases.filter(
    (item) => item.status === "Manufacturing"
  ).length

  const deliveredCount = designerCases.filter(
    ...
    >
      ...
      >
        ...
        {!loadingPage && selectedDesignerId && (
          ...
          >
            <StatCard title="My Active Cases" value={activeCasesCount} />
